self.onmessage = function(e) {
    ...
    // إنشاء نسخة احتياطية من البيانات الأصلية (هذا هو مفتاح الحل)
    const rawPixels = new Uint8ClampedArray(inputPixels);

    // مصفوفة المرشح (Kernel) لزيادة الحدة
    const sharpenKernel = [
        0, -1, 0,
        -1, 5, -1,
        0, -1, 0
    ...
    // تطبيق التباين أولاً (يتم تطبيقه على inputPixels فقط)
    const contrast = 1.2; 
        ...
        for (let x = kernelRadius; x < width - kernelRadius; x++) {
            ...
            for (let ky = -kernelRadius; ky <= kernelRadius; ky++) {
                for (let kx = -kernelRadius; kx <= kernelRadius; kx++) {
                    ...
                    // هنا نستخدم البيانات التي طبقنا عليها التباين (inputPixels)
