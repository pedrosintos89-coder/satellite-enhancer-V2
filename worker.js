...
function getAverageColorFromBuffers(groundBuffers) {
    ...
    // المرور على كل مصفوفة بيانات (صورة)
    for (const buffer of groundBuffers) {
        // تحويل البفر إلى مصفوفة بكسلات
    ...
    // حساب متوسط الألوان الموزون لجميع البكسلات المجمعة
    return {
...
self.onmessage = function(e) {
    ...
    // 2. تطبيق مرشح الحدة القوي (Sharpening Kernel)
    const sharpenKernel = [
        -1, -1, -1,
        -1, 9, -1,
        -1, -1, -1
    ...
    for (let y = kernelRadius; y < height - kernelRadius; y++) {
        ...
        for (let x = kernelRadius; x < width - kernelRadius; x++) {
            ...
            // هذا الإجراء يضمن أن اللون المحسوب للحدة يتأثر باللون المرجعي (avgGroundColor)
            const colorBlendFactor = 0.5;
